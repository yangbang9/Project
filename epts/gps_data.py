...
#     # 텍스트 파일 읽기 (인코딩 지정)
#     with open('./data/data6.txt', 'r', encoding='UTF-8') as file:
    ...
    for line in lines:
        ...
        if re.match(r'^time :', line):
            if current_data:
                data.append(current_data)
            current_data = {}
            current_data['time'] = re.search(r'\d+\.\d+', line).group()
        elif re.match(r'^Latitude :', line):
            current_data['lat'] = float(re.search(r'-?\d+\.\d+', line).group())
        elif re.match(r'^Longitude :', line):
            current_data['long'] = float(re.search(r'-?\d+\.\d+', line).group())
        elif re.match(r'^accel\(x, y, z\) :', line):
            current_data['accel'] = tuple(map(float, re.findall(r'-?\d+\.\d+', line)))
        elif re.match(r'^gyro\(x, y, z\) :', line):
            current_data['gyro'] = tuple(map(float, re.findall(r'-?\d+\.\d+', line)))
        elif re.match(r'^mag\(x, y, z\) :', line):
            current_data['mag'] = tuple(map(float, re.findall(r'-?\d+\.\d+', line)))
        elif re.match(r'^Speed \(m/s\):', line):
            current_data['speed (m/s)'] = float(re.search(r'-?\d+\.\d+', line).group())
